...
export function createApp() {
  ...
  app.post('/api/generate', (req: Request, res: Response) => {
    const b = req.body as Partial<VideoConfig>;
    ...
    const config: Omit<VideoConfig, 'id'> = {
      ...
      fps:        ([24,30,60] as const).includes(Number(b.fps) as never) ? Number(b.fps) as VideoConfig['fps'] : 30,
      duration:   Math.max(1, Math.min(3600, Number(b.duration) || 10)),
  ...
  app.delete('/api/jobs/:id', (req: Request, res: Response) => {
    if (!deleteJob(req.params.id)) { res.status(400).json({ ok: false, error: 'Cannot delete' }); return; }
